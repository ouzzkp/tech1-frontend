...
function Profile() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/hr/api/employees/get?id=${userID}`, {
          headers: {
      ...
      <div className="profile-wrapper">
          <div className="profile-page">
              ...
              <div className="profile-info-section">
                  ...
                  <h2 className="profile-email">{userProfile.emailAddress}</h2>
                  <h3 className="profile-department">{userProfile.jobTitle}</h3>
                  <p className="profile-bio">{userProfile.address}</p>
              </div>
          </div>
      </div>
  );
